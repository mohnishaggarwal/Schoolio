...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        var ZoomID = req.body.zoomid
        await deleteDoc(doc(db, "Zoom", ZoomID));

        res.end(`Successfully deleted ${ZoomID}`)
    }
}

// curl -X POST http://localhost:3000/api/other/delete_zoom  -H "Content-Type: application/json"  -d '{"zoomid": "1xkTaVOUU1CikZvTRcQs"}'